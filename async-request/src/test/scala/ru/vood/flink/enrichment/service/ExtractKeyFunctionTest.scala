...
import ru.vood.flink.enrichment.TestConst.globalIdStreamProperty
import ru.vood.flink.enrichment.service.KeyEnrichmentGlobalIdServiceTest._
import ru.vood.flink.enrichment.service.dto.KeyedCAData
import ru.vood.flink.enrichment.utils.config.enrich.intf.InputFormatEnum.FlatJsonFormat
import ru.vood.flink.enrichment.utils.config.enrich.{GlobalIdEnrichProperty, KeySelectorProp}
...
class ExtractKeyFunctionTest extends AnyFlatSpec {
  ...
  it should " validate is err, no id" in {
    ...
    val outDtoWithErrors = OutDtoWithErrors[JsValue](
      serviceDataDto,
      Some("ru.vood.flink.enrichment.service.ExtractKeyFunction"),
